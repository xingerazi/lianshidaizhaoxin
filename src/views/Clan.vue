<template>
  <el-card class="questionbox">
    ...
    <el-card class="question">
      <h3 style="margin-bottom: 30px">1. 三连击</h3>
      <p>
        <span>题目要求: </span
    ...
    <el-card class="question">
      <h3 style="margin-bottom: 30px">2.塔塔开！</h3>
      <p>
        <span>题目背景：</span
...
于是，你需要完成一个可以将收到的信息完整输出的程序。
    ...
    <el-card class="question">
      ...
      <p>
        <span>输出格式: </span>第二行有 n 个整数，第 i 个整数表示序列的第 i 个数字 a_i。
      </p>
      <pre>
    ...
    <el-card class="question">
      <h3 style="margin-bottom: 30px">4.线性筛素数</h3>
      <p>
        <span>题目背景: </span
...
提示：如果你使用  `cin` 来读入，建议使用 `std::ios::sync_with_stdio(0)` 来加速。
    ...
    <el-card class="question">
      <h3 style="margin-bottom: 30px">5.最大公约数和最小公倍数问题</h3>
      <p>
        <span>题目描述: </span
...
<p>试求：满足条件的所有可能的 P, Q 的个数</p>
      ...
      <p><span>提示：</span>P,Q 有4 种：
3,60；
15, 12；
12, 15；
60, 3。</p>
<p>对于 100% 的数据，x<sub>0</sub> ,y<sub>0</sub>：[2,10<sup>5</sup> ]</p>
    </el-card>

    <h2 style="margin-top: 30px">C语言 进阶</h2>
    <el-divider></el-divider>
    <el-card class="question">
      <h3 style="margin-bottom: 30px">6.硬币翻转</h3>
      <p>
        <span>题目描述：</span
        >在桌面上有一排硬币，共N枚，每一枚硬币均为正面朝上。现在要把所有的硬币翻转成反面朝上，规则是每次可翻转任意N-1枚硬币。

<p><span style="color: rgb(255,255,255);">题目描述：</span> 求一个最短的操作序列（将每次翻转N-1枚硬币成为一次操作）。</p>


      </p>
      <p><span>输入格式：</span>一个自然数 N（N 为不大于 100 的偶数）。</p>
      <p><span>输出格式：</span>第一行包含一个整数 $S$，表示最少需要的操作次数。接下来的S行每行分别表示每次操作后桌上硬币的状态。</p>
      <p><span style="color: rgb(255,255,255);">输出格式：</span>（一行包含N个整数表示每个硬币的状态，0表示正面向上,1表示反面向上。不允许输出多余空格。</p>
      <p><span style="color: rgb(255,255,255);">输出格式：</span>对于有多种操作方案的情况，则只需操作的字典序最小输出一种。</p>
      <p><span style="color: rgb(255,255,255);">输出格式：</span>操作的字典序是指，对于一次操作中的每个位置，1表示翻转，0表示不反转。</p>
      <p><span style="color: rgb(255,255,255);">输出格式：</span>但是需要你输出的是每一次操作完的状态，0表示正面朝上，1表示反面朝上。</p>
      <pre>
      ...
      >
      <p><span>提示：</span>有时候，反着思考某些条件，也是一种优秀的解题方法。</p>
    </el-card>
    <el-card class="question">
      <h3 style="margin-bottom: 30px">7.独木桥</h3>
      <p>
        <span>题目背景：</span
      ...
      <pre>
        ...
        1 3
        样例输出 #1
        2 4                //提示：对于 100% 的数据，满足初始时，没有两个士兵同在一个坐标，1 <=  L <= 5 * 10^3，且数据保证 N<= L。
...
于是，他预知了所有情况下自己能逃命的方案。
      ...
      >
      <p>    提示:   [数据范围]： 对于100%的数据，n:[6,13] 。试试深度优先搜索！   </p>
    </el-card>
...
作为名侦探，你下定决心要解决这个奇怪的事件。。
      ...
      <p><span style="color: rgb(255,255,255);">题目描述：</span><span>如：gcd(31,13)=1，那么13和31都是lsdzx数。</span>
</p>
<p><span style="color: rgb(255,255,255);">题目描述：</span>
...
请注意，每个lsdzx数都会小于一个数k
      ...
      <pre>
        ...
        输入格式:
        三个数n,m,k
        接下来一排n个数，a_1,a_2....a_n代表数轴上每个数的值
        接下来m排每排一组l,r代表一次询问
    ...
    <el-card class="question">
      <h3 style="margin-bottom: 30px">10.彼岸花开七色天</h3>
      <p>
        <span>题目背景：</span>
        彼岸花开七色天，花开花落一千年
      </p>
...
的。</p><p><img src="../assets/10c.png" alt=""></p>
      <pre>

        输入格式： 
        一个数n        
        接下来n排，两个数l,r,请求出它们的最近公共祖先
      ...
      >
      <p> <span style="display: block;"> 提示:</span>  <img src="../assets/10cc.png" alt="" style="height: 60px;">  </p>
    </el-card>
  </el-card>
</template>
