...
<p>求一个最短的操作序列（将每次翻转N-1枚硬币成为一次操作）。</p>


      </p>
      <p><span>输入格式：</span>一个自然数 N（N 为不大于 100 的偶数）。</p>
      <p><span>输出格式：</span>第一行包含一个整数 $S$，表示最少需要的操作次数。接下来的S行每行分别表示每次操作后桌上硬币的状态。</p>
      <p>（一行包含N个整数表示每个硬币的状态，0表示正面向上,1表示反面向上。不允许输出多余空格。</p>
      <p>对于有多种操作方案的情况，则只需操作的字典序最小输出一种。</p>
      <p>操作的字典序是指，对于一次操作中的每个位置，1表示翻转，0表示不反转。</p>
      <p>但是需要你输出的是每一次操作完的状态，0表示正面朝上，1表示反面朝上。</p>
      <pre>
    ...
    样例输出 #1
    2 4                
    提示：对于 100% 的数据，满足初始时，没有两个士兵同在一个坐标，
          1 <=  L <= 5 * 10^3，且数据保证 N<= L。
...
的。</p><p><img src="../assets/10c.png" alt="" style="width: 100%; margin-left: 0;"></p>
      <pre>
